...
{
	public partial class UsersCMController : CMBaseApiController
	{
			private const string GET_LIST_ROUTE_NAME = "UsersCMList";
			private const int maxPageSize = 100;

			private GenericFactory<entCM.User, dtoCM.User> _factory 
				= new GenericFactory<entCM.User, dtoCM.User>();

		public UsersCMController() : base()
		{
		}

		public UsersCMController(ILoggingService log, ICMRepository repository)
		: base(log, repository)
		{
		}
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				dbItems = dbItems.ApplySort(sort ?? (typeof(entCM.User).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpPatch]
		[VersionedRoute(template: "Users/{userId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int userId, [FromBody] JsonPatchDocument<dtoCM.User> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		public async Task<IHttpActionResult> Post([FromBody] dtoCM.User dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		[HttpPut]
		[VersionedRoute(template: "Users/{userId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int userId, [FromBody] dtoCM.User dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref MSC.ConferenceMate.Repository.Entities.CM.User newDBItem, ref IRepositoryActionResult<entCM.User> result);

		partial void RunCustomLogicAfterUpdatePatch(ref MSC.ConferenceMate.Repository.Entities.CM.User updatedDBItem, ref IRepositoryActionResult<entCM.User> result);

		partial void RunCustomLogicAfterUpdatePut(ref MSC.ConferenceMate.Repository.Entities.CM.User updatedDBItem, ref IRepositoryActionResult<entCM.User> result);

		partial void RunCustomLogicOnGetEntityByPK(ref entCM.User dbItem, int userId, int numChildLevels);

		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entCM.User> dbItems, ref List<string> filterList);
	}
}